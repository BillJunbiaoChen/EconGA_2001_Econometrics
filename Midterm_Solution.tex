...
\begin{align*}
    ...
    & = (G'\Omega^{-1}G)^{-1}
\end{align*}
Since $\Omega := \mathbb{E}[ZZ'\epsilon^2] = \mathbb{E}[ZZ'\mathbb{E}[\epsilon^2|Z]] = \sigma_0^2(Z) \mathbb{E}(ZZ')$ by the assumption. 
$G = \mathbb{E}[ZX']$.
Plugging them into the equation above, we have the asymptotic variance given by 
\begin{align}
    (G'WG)^{-1}G'W\Omega W G(G'WG)^{-1} 
    & = \sigma_0^2(Z) \bigg[\mathbb{E}[XZ'] \mathbb{E}(ZZ')^{-1} \mathbb{E}[ZX']\bigg]^{-1}
\end{align}

\subsection{(b) Answer}
I propose consistent estimators of $W^*$ and $\Sigma^*$ as follows.
\begin{align}
  \hat{W}^* & = \left(\frac{1}{n} \sum_{i=1}^{n} Z_i Z_i' \right)^{-1} 
\end{align}
...
\end{align}
where $\hat{G} = \frac{1}{n} \sum_{i=1}^n Z_i X_i'$.

\noindent \textbf{Justifications for my proposals:}
\begin{itemize}
  \item Under the assumption of conditional homoskedasticity, the generic form of 
  \[\hat{W}^* = 
  ...
  \item In fact, the propose $\hat{W}^*$ above is the same as the one used in 2SLS estimator, which is efficient
...
Finally, by Slusky theorem, we have 
\[
...
\begin{align}
  \hat{\Sigma}^* & = \bigg[\hat{G}'\hat{W}^* \hat{G}\bigg]^{-1}, 
\end{align}
where $\hat{W}^* = \left(\frac{1}{n} \sum_{i=1}^{n} Z_i Z_i' (Y_i - X_i' \hat{\beta}_{\text{2SLS}})^2 \right)^{-1}$.
$\hat{\beta}_{\text{2SLS}} = \left( X'P_Z X \right)^{-1} X'P_Z Y$.
$\hat{G} = \frac{1}{n} \sum_{i=1}^n Z_i X_i'$.
...
\begin{itemize}
  ...
  \[
    \left(\hat{W}^*\right)^{-1} = \frac{1}{n} \sum_{i=1}^{n} Z_i Z_i' (Y_i - X_i' \hat{\beta}_{\text{2SLS}})^2 
    \rightarrow_p \mathbb{E}[Z Z' \epsilon^2] = \Omega.
  \]
  \item Finally, since $\hat{G} \rightarrow_p \mathbb{E}[ZX']$, we can conclude that $\hat{\Sigma}^*$ is consistent.
\end{itemize}
