...
\begin{align}
  \hat{W} & = \hat{\sigma}^2 \left(\frac{1}{n} \sum_{i=1}^{n} Z_i Z_i' \right)^{-1} = \hat{\sigma}^2 (\bm{Z}'\bm{Z}/n)^{-1},
\end{align}
where $Z_i$ is $p\times 1$ vector (observation), and $\bm{Z} = (Z_1, Z_2, \dots, Z_n)'$ is $n \times p$ matrix.
