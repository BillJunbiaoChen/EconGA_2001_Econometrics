...
Show that under the conditions of monotonicity and $(X_{i},D_{i}) \perp Z_{i}$:
$$
\mathbb{E}[D_{i}(1-Z_{i})X_{i}]=\mathbb{E}[X_{i}|D_{0i}=1,D_{1i}=1]P(D_{0i}=1,D_{1i}=1)P(Z_{i}=0)
$$
...
Show that under the same conditions:
$$
\mathbb{E}[(1-D_{i})Z_{i}X_{i}]=\mathbb{E}[X_{i}|D_{0i}=0,D_{1i}=0]P(D_{0i}=0,D_{1i}=0)P(Z_{i}=1)
$$

The term $(1-D_i)Z_iX_i$ is non-zero only when $D_i=0$ and $Z_i=1$.
\begin{align*}
\mathbb{E}[(1-D_{i})Z_{i}X_{i}] &= \mathbb{E}[X_{i} | D_i=0, Z_i=1] \cdot P(D_{i}=0|Z_{i}=1) \cdot P(Z_{i}=1)
\end{align*}
...
Show that for the function $w_{i}:=1-\frac{D_{i}(1-Z_{i})}{P(Z_{i}=0)}-\frac{(1-D_{i})Z_{i}}{P(Z_{i}=1)}$,
$$
\mathbb{E}[w_{i}X_{i}]=\mathbb{E}[X_{i}|D_{0i}=0,D_{1i}=1]P(D_{0i}=0,D_{1i}=1)
$$

Expand $\mathbb{E}[w_{i}X_{i}]$:
$$
...
\subsection{c. Answer}
As $T \rightarrow$, the relative weight shrinks to zero.
